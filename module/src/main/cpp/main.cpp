...
EXPORT void nativeForkAndSpecializePre(
    ...
    sHookEnable = equals(cNiceName, "com.eg.android.AlipayGphone");
    if (cNiceName) {
        free(cNiceName);
    }
}
...
EXPORT int nativeForkAndSpecializePost(JNIEnv *env, jclass clazz, jint res) {
    if (res == 0) {
        ...
        if (sHookEnable) {
            ...
            const char *dexPath = "/data/local/tmp/libxfingerprint_pay_alipay.dex";
            if (access(dexPath, 0) != 0) {
                dexPath = "/system/framework/libxfingerprint_pay_alipay.dex";
            }
            loadDex(env,
